...
function getCategoryEmoji(category: string) {
  return CATEGORY_MAP[category]?.emoji || '‚ú?
}
...
async function reclassifyArticles() {
  ...
  try {
    ...
    if (dryRun.total === 0) {
      reclassifyMessage.value = 'All articles are already properly categorized! üéâ'
    ...
    reclassifyMessage.value = `‚ú?Setup Complete! Processed ${processedCount} articles. Fixed ${changedCount} categories.`
    await refreshCategoryStats()
...
async function deleteAllArticles() {
  // Double confirmation
  if (!confirm('‚ö†Ô∏è This will DELETE ALL ARTICLES!\n\nAre you sure you want to start fresh?')) {
    return
  }
